...
-- 定理の実際の適用例
example : MyEven (2 * 2) := even_square_main 2 (by use 2; norm_num)
example : MyEven (4 * 4) := even_square_direct 4 (by use 8; norm_num)
example : MyEven (6 * 6) := even_square_simple 6 (by use 18; norm_num)
...
example (n : ℤ) : 
  (MyEven (n * n) → MyEven n) ∧ 
  (MyEven (n * n) → MyEven n) ∧ 
  (MyEven (n * n) → MyEven n) := by
  exact ⟨even_square_main n, even_square_direct n, even_square_simple n⟩

-- 対偶の確認
example (n : ℤ) : MyOdd n → MyOdd (n * n) := odd_square n

-- 基本性質の確認
example : MyEven 0 := zero_even
example : MyEven 2 := two_even
example : MyOdd 1 := one_odd

...
-/
  ...
  by
    ...
    have hγE0 : γE t0 = e₀ := by
      ...
      -- Compare with the distinguished sheet through e₀ at the base point.
      have Hsheet : (d.sheet i ⟨p e₀, hUE₀⟩).1 = e₀ := by
        -- from the sheet-through lemma, then take values in E
        have := EvenlyCoveredAt.sheet_through (p:=p) (b:=b₀) d e₀ hUE₀
        simpa [i] using congrArg Subtype.val this
      -- Change the base from `p e₀` to `b₀` using `h₀`.
      have Hbase_b0 : (d.sheet i ⟨b₀, by simpa [h₀] using d.memU⟩).1 = e₀ := by
        ...
        simpa [EvenlyCoveredAt.sheet, Heq] using Hsheet
      -- Now change from `b₀` to the true start `γ I0` using `γ.source`.
      have hγ0 : γ I0 = b₀ := by simpa using (path_source_I0 (γ := γ))
      have Hstart : (d.sheet i ⟨γ I0, hU0⟩).1 = e₀ := by
        have Heq : (⟨γ I0, hU0⟩ : Subtype fun x : B => x ∈ d.U)
          = ⟨b₀, by simpa [h₀] using d.memU⟩ := by
          apply Subtype.ext; simpa [hγ0]
        simpa [EvenlyCoveredAt.sheet, Heq] using Hbase_b0
      -- unfold the definition of γE at 0
      simpa [γE, EvenlyCoveredAt.liftAlong, EvenlyCoveredAt.toSubtype,
              EvenlyCoveredAt.inclE, EvenlyCoveredAt.sheetMap, EvenlyCoveredAt.sheet]
        using Hstart
  ...
  lemma liftPathLocal_map_apply
    ...
    classical
    -- expand map evaluation
    simp [liftPathLocal, path_map_apply, EvenlyCoveredAt.liftAlong]
    -- goal reduces to the bundled β for `liftAlong` evaluated at `t`
    ...
    -- simplify the RHS to `γ t`
    simpa [EvenlyCoveredAt.toSubtype, EvenlyCoveredAt.inclB, ContinuousMap.comp_apply]
      using this
  ...
  noncomputable def liftPathLocalOn
      ...
      (a b : unitInterval)
      (eₐ : E) (hₐ : p eₐ = γ a)
      (hγU : ∀ t : unitInterval, γ (Bourbaki.TopologyB.PathTools.segMap a b t) ∈ d.U) :
      Path eₐ ((d.liftAlong h (d.sheetIndexAt eₐ (by
        have : γ a ∈ d.U := by
          simpa using hγU I0
        simpa [hₐ] using this))
        (γ.toContinuousMap.comp (Bourbaki.TopologyB.PathTools.segMap a b))
  ...
  by
    ...
    have γab : Path (γ a) (γ b) := Bourbaki.TopologyB.PathTools.subpath γ a b
    have hU : ∀ t : unitInterval, γab t ∈ d.U := by
      intro t; simpa [γab, Bourbaki.TopologyB.PathTools.subpath, ContinuousMap.comp_apply]
        using hγU t
    -- Apply the local lifting on `γab`.
    simpa [γab, Bourbaki.TopologyB.PathTools.subpath]
      using liftPathLocal (p:=p) h d γab eₐ hₐ hU

  /-- β-rule (bundled) for the localized lift on `[a,b]`. -/
  lemma liftPathLocalOn_map
      (h : CoveringMap p) {b₀ b₁ : B}
      (d : EvenlyCoveredAt p b₀)
      (γ : Path b₀ b₁)
      (a b : unitInterval)
      ...
      (liftPathLocalOn (p:=p) h d γ a b eₐ hₐ hγU).map (f := p) h.continuous
        = Bourbaki.TopologyB.PathTools.subpath γ a b := by
    classical
    -- Reduce to the β-rule pointwise and ext
    ext t
    -- use local β at the subpath
    have := liftPathLocal_map_apply (p:=p) h d (Bourbaki.TopologyB.PathTools.subpath γ a b)
      eₐ hₐ (by intro t'; simpa [Bourbaki.TopologyB.PathTools.subpath, ContinuousMap.comp_apply] using hγU t') t
    simpa [Bourbaki.TopologyB.PathTools.subpath, path_map_apply]
      using this
  ...
  noncomputable def liftPathTwo
      ...
      (eₐ : E) (hₐ : p eₐ = γ a)
      (hγU₁ : ∀ t : unitInterval, γ (Bourbaki.TopologyB.PathTools.segMap a c t) ∈ d₁.U)
      (hγU₂ : ∀ t : unitInterval, γ (Bourbaki.TopologyB.PathTools.segMap c b t) ∈ d₂.U) :
      Path eₐ ((liftPathLocalOn (p:=p) h d₂ γ c b
        ((liftPathLocalOn (p:=p) h d₁ γ a c eₐ hₐ hγU₁) I1)
        (by
          -- equality `p (Γ₁ 1) = γ c` via apply-level local β at endpoint
          have hβ := liftPathLocal_map_apply (p:=p) h d₁ (Bourbaki.TopologyB.PathTools.subpath γ a c)
            eₐ hₐ (by intro t; simpa [Bourbaki.TopologyB.PathTools.subpath, ContinuousMap.comp_apply] using hγU₁ t) I1
          simpa [Bourbaki.TopologyB.PathTools.subpath, path_map_apply] using hβ)
        hγU₂) I1) :=
  by
    classical
    ...
    have hcstart : p e_c = γ c := by
      ...
      have := liftPathLocal_map_apply (p:=p) h d₁ (Bourbaki.TopologyB.PathTools.subpath γ a c)
        eₐ hₐ (by intro t; simpa [Bourbaki.TopologyB.PathTools.subpath, ContinuousMap.comp_apply] using hγU₁ t) I1
      simpa [Γ₁, Bourbaki.TopologyB.PathTools.subpath, path_map_apply] using this
  ...
  structure PathCover {b₀ b₁ : B} (p : E → B) (γ : Path b₀ b₁) where
    ...
    inU : ∀ i : Fin n, ∀ t : unitInterval,
      γ (Bourbaki.TopologyB.PathTools.segMap (pts i.castSucc) (pts i.succ) t) ∈ (charts i).U
  ...
  by
    ...
    | (Nat.succ k), hk =>
      ...
      (build k hk').trans (Bourbaki.TopologyB.PathTools.subpath γ (cov.pts i.castSucc) (cov.pts i.succ))
    ...
    by
      ...
      · intro hkSucc
        ...
        have hend : p (Γseg I1) = γ (cov.pts i.succ) := by
          ...
          have := congrArg (fun (δ : Path _ _) => δ I1) this
          simpa using this

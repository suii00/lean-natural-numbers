...
/- Product object and universal property data -/
...
def pair {X Y Z : TopPair} (f : Hom Z X) (g : Hom Z Y) : Hom Z (X ⨯ Y) := by
  classical
  refine ⟨(fun z => (f.1 z, g.1 z)), ?_⟩
  let _ : TopologicalSpace X.E := X.τ
  let _ : TopologicalSpace Y.E := Y.τ
  let _ : TopologicalSpace Z.E := Z.τ
  have _ : TopologicalSpace (X.E × Y.E) := instTopologicalSpaceProd
  exact f.2.prodMk g.2

/- Universal property of the product: projections after pairing and uniqueness -/
...
theorem pair_unique {X Y Z : TopPair} {h : Hom Z (X ⨯ Y)} {f : Hom Z X} {g : Hom Z Y}
    (h₁ : Hom.comp (proj₁ X Y) h = f)
    (h₂ : Hom.comp (proj₂ X Y) h = g) :
    h = pair f g := by
  classical
  apply Subtype.ext
  funext z
  -- Compare components at z using the hypotheses
  have h₁' := congrArg (fun (k : Hom Z X) => (k : Z.E → X.E) z) h₁
  have h₂' := congrArg (fun (k : Hom Z Y) => (k : Z.E → Y.E) z) h₂
  -- Build equality of pairs from component equalities
  refine Prod.ext ?_ ?_
  · simpa [Hom.comp_apply] using h₁'
  · simpa [Hom.comp_apply] using h₂'

@[simp] theorem pair_eta {X Y Z : TopPair} (h : Hom Z (X ⨯ Y)) :
    pair (Hom.comp (proj₁ X Y) h) (Hom.comp (proj₂ X Y) h) = h := by
  classical
  -- Apply uniqueness with f := proj₁ ∘ h and g := proj₂ ∘ h
  refine (pair_unique (X:=X) (Y:=Y) (Z:=Z) (h:=h) ?_ ?_)
  · rfl
  · rfl
/-********************
...
def eval (X Y : TopPair) : (C X Y × X.E) → Y.E := fun p => p.1.1 p.2
theorem eval_continuous (X Y : TopPair) : Continuous (eval X Y) := by
  classical
  let _ : TopologicalSpace X.E := X.τ
  let _ : TopologicalSpace Y.E := Y.τ
  let _ : TopologicalSpace (C X Y) := instTopC X Y
  -- inclusion C(X,Y) → (X → Y) is continuous by induced topology
  have hIncl : Continuous (fun f : C X Y => (f.1 : X.E → Y.E)) := continuous_induced_dom
  -- build (C×X) → ((X→Y)×X)
  have hProd : Continuous (fun p : (C X Y × X.E) => ((p.1.1 : X.E → Y.E), p.2)) := by
    refine (hIncl.comp (continuous_fst)).prodMk continuous_snd
...
def uncurry {X Y Z : TopPair} (F : C X (C Y Z)) : C (X ⨯ Y) Z := by
  classical
  -- underlying function
  refine ⟨(fun p => (F.1 p.1).1 p.2), ?_⟩
  -- prove continuity via evaluation continuity
  let _ : TopologicalSpace X.E := X.τ
  let _ : TopologicalSpace Y.E := Y.τ
  let _ : TopologicalSpace Z.E := Z.τ
  let _ : TopologicalSpace (C Y Z) := instTopC Y Z
  let _ : TopologicalSpace (X.E × Y.E) := instTopologicalSpaceProd
  -- map into ((Y→Z) × Y)
  have hIncl : Continuous (fun f : C Y Z => (f.1 : Y.E → Z.E)) := continuous_induced_dom
  have h3 : Continuous (fun p : (X.E × Y.E) => ((F.1 p.1 : Y.E → Z.E), p.2)) := by
...
structure Pair where
  ...
  grp : Group E
  tgrp : by
    let _ : TopologicalSpace E := τ
    let _ : Group E := grp
    exact TopologicalGroup E

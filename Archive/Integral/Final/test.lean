...
theorem gaussian_bounded (R : ℝ) (hR : 0 < R) :
  ...
  -- Pointwise lower bound on [0, r]: exp(-1) ≤ exp(-x^2)
  ...
  have h_sub_pos : 0 < ∫ x in (0:ℝ)..r, Real.exp (-x^2) := by
    ...
    -- Left side equals (r - 0) * exp(-1)
    have h_const : ∫ x in (0:ℝ)..r, (Real.exp (-1) : ℝ)
        = (r - (0 : ℝ)) * (Real.exp (-1)) := by
      -- integral_const: (b - a) • c; over ℝ, • = *
      simpa using
        (intervalIntegral.integral_const (a := (0:ℝ)) (b := r) (c := (Real.exp (-1) : ℝ)))
    -- Strictly positive since exp(-1) > 0 and r > 0
  ...
  -- Enlarge interval from [0, r] to [-R, R] using indicator/AE comparison
  -- Show inclusion of sets and use AE monotonicity of indicators
  have h_subset : Set.Ioc (0:ℝ) r ⊆ Set.Ioc (-R) R := by
    ...
    exact ⟨hgtmR, hxleR⟩
  -- Rewrite both interval integrals as indicator integrals on ℝ
  have h_rewrite_big :
  ...
  have h_rewrite_small :
    ...
    simpa using (this.trans
      ((integral_indicator (μ := volume) (s := Set.Ioc (0:ℝ) r)
        (f := fun x : ℝ => Real.exp (-x^2)) (measurableSet_Ioc)).symm))
  -- Indicator monotonicity from set inclusion and nonnegativity
  have h_ind_le : ∀ x : ℝ,
  ...
  -- Compare the two indicator integrals (AE monotonicity)
  have h_expand :
      ∫ x in (0:ℝ)..r, Real.exp (-x^2)
        ≤ ∫ x in (-R)..R, Real.exp (-x^2) := by
    -- Both sides as integrals over volume of indicators
    have h_big_int : Integrable (fun x : ℝ => (Set.indicator (Set.Ioc (-R) R)
      (fun x => Real.exp (-x^2)) x)) := by
      -- derived from Gaussian integrability via indicator
    ...
    simpa [h_rewrite_small, h_rewrite_big]
      using hmono
  -- Finally, strict positivity on the big interval
  have h_pos_integral : 0 < ∫ x in (-R)..R, Real.exp (-x^2) :=
    lt_of_lt_of_le h_sub_pos h_expand

  -- Upper bound via AE comparison with the full Gaussian integral (≤ √π < 2√π)
  have hle : (-R) ≤ R := by linarith
  -- Rewrite the interval integral as an integral w.r.t. a restricted measure
  have h_restrict :
      ∫ x in (-R)..R, Real.exp (-x^2)
        = ∫ x, Real.exp (-x^2) ∂(volume.restrict (Set.Ioc (-R) R)) := by
    simpa using (intervalIntegral.integral_of_le (a := -R) (b := R)
  ...
  -- Convert restricted-measure integral into an indicator integral on ℝ
  have h_indicator :
      ∫ x, Real.exp (-x^2) ∂(volume.restrict (Set.Ioc (-R) R))
        = ∫ x, (Set.indicator (Set.Ioc (-R) R) fun x => Real.exp (-x^2)) x ∂volume := by
    -- standard identity between restrict and indicator
    simpa using (integral_indicator (μ := volume)
      (s := Set.Ioc (-R) R) (f := fun x : ℝ => Real.exp (-x^2))
      (measurableSet_Ioc)).symm

  -- Pointwise AE inequality: indicator ≤ identity since the integrand is nonnegative
  ...
  -- Integrability of the full Gaussian on ℝ and of its indicator restriction
  have h_gauss_int : Integrable (fun x : ℝ => Real.exp (-x^2)) := by
    -- Use the general Gaussian integrability for b = 1
    simpa [neg_mul, one_mul] using
      (integrable_exp_neg_mul_sq (b := (1 : ℝ)) (by norm_num))
  ...
  -- Compare indicator integral with the full integral (AE monotone)
  have h_bound_le :

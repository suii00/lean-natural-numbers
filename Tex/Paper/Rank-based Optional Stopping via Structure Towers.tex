...
We now bridge the gap between structure towers and probability theory. The relationship between the general definitions and our specific bounded setting is summarized as follows:
\[
  \text{Stopping Time } (\Nat \cup \{\infty\}) \xrightarrow{\text{Bounded}} \text{Stopping Time } (\Nat) \iff \text{Covering Rank } (\Nat)
\]
Throughout the rest of this paper, we assume $\tau$ is a \textbf{bounded stopping time}. That is, there exists $K \in \Nat$ such that $\tau(\omega) \le K$ for all $\omega$. Consequently, $\tau$ corresponds to a covering rank function $\rank_\tau: \Omega \to \Nat$.
...
\begin{enumerate}
    ...
    \end{align*}
    Crucially, the term $\Ind{\tau \le n} M_\tau$ is $\mathcal{F}_n$-measurable. This is because it can be decomposed as a finite sum $\sum_{k=0}^n \Ind{\tau=k} M_k$, where each term is the product of an $\mathcal{F}_k$-measurable event ($\{\tau=k\} \in \mathcal{F}_k \subseteq \mathcal{F}_n$) and an $\mathcal{F}_k$-measurable variable ($M_k \in \mathcal{F}_k \subseteq \mathcal{F}_n$). Thus, it factors out of the conditional expectation:
    \begin{align*}
...
A key feature of our formalization is the use of \texttt{abbrev} to define rank-theoretic concepts. This ensures \textbf{definitional equality} with standard Mathlib concepts, allowing us to reuse existing theorems without manual type casting.

For example, \texttt{rankStoppedProcess} is defined as an alias for \texttt{Martingale.stoppedProcess}. This allows proofs like the following to succeed using only \texttt{exact} or \texttt{simp}:

\begin{verbatim}
-- Because rankStoppedProcess is definitionally equal to stoppedProcess,
-- we can apply Mathlib's integral_condExp directly.
theorem rankOptionalStopping_bounded ... :
  Integral (rankStoppedProcess M tau n) = Integral (M 0) := by
  -- The goal is unified with the standard martingale theorem

...
This identification allows a discrete, combinatorial proof of the bounded OST in the finite setting.
...
$\NN = \{0,1,2,\ldots\}$ denotes the natural numbers.
...
Given $\rho: X \to \NN$, define $\Tower_\rho$ by
\[
\layer_n = \{x \in X : \rho(x) \leq n\}, \quad \minLayer(x) = \rho(x).
\]
\end{definition}
...
For $\rho: X \to \NN$, $\minLayer_{\Tower_\rho}(x) = \rho(x)$.
\end{lemma}

\noindent\textbf{Proof sketch.}
By Definition~\ref{def:tower-from-rank}, $\minLayer(x) = \rho(x)$ directly.
\qed
...
Given $\Tower$, define $\rank_\Tower: X \to \NN$ by $\rank_\Tower(x) = \minLayer(x)$.
\end{definition}
...
(2): Let $x \in X$. If $x \in \layer_n$, then $\minLayer(x) \leq n$ by minimality.
Conversely, if $\minLayer(x) \leq n$, then $x \in \layer_{\minLayer(x)}$ by definition of $\minLayer$ (\texttt{minLayer\_mem}), and $\layer_{\minLayer(x)} \subseteq \layer_n$ by monotonicity, hence $x \in \layer_n$.
\qed

This bijection justifies identifying towers with rank functions.
In Lean, this is:

\texttt{noncomputable def towerFromRank (rho : X -> Nat) : StructureTowerWithMin}
...
The filtration $\Filtration$ itself can induce a tower structure under additional assumptions.
Assume $\Filtration$ is \emph{eventually separating}: for each $\omega \in \Omega$, there exists $n \in \NN$ such that $\{\omega\} \in \Filtration_n$.
Then we can define the \emph{first measurable time}:
\[
\mathrm{firstMeas}(\omega) = \min\{n \in \NN : \{\omega\} \in \Filtration_n\}.
\]
This yields a rank function on $\Omega$, with layers equal to the sets of singletons measurable at time $n$.

\begin{remark}
In finite probability spaces with coarse filtrations, singletons may never become measurable.
The eventually-separating assumption ensures that $\mathrm{firstMeas}$ is well-defined.
For general filtrations, one can instead define ranks on \emph{atoms} (minimal non-empty measurable sets) rather than singletons.
\end{remark}
...
We have demonstrated that the Optional Stopping Theorem admits a discrete, combinatorial formulation via rank functions and structure towers, where stopping times are rank functions and stopping sets are tower layers.
...
\item \textbf{Finitary simplicity:} Rank functions are elementary objects (maps to $\NN$), avoiding measure-theoretic subtleties in the discrete setting.
...
\textit{arXiv:2309.06311}, 2023.

\bibitem{coq-prob}
R.~Affeldt, C.~Cohen, and D.~Rouhling.
Formalization Techniques for Asymptotic Reasoning in Classical Analysis.
\textit{Journal of Formalized Reasoning}, 11(1):43--76, 2018.

\bibitem{lean-mathlib}
The mathlib Community.
The Lean mathematical library.
In \textit{CPP 2020: Proceedings of the 9th ACM SIGPLAN International Conference on Certified Programs and Proofs}, pages 367--381, 2020.
Also available at \url{https://github.com/leanprover-community/mathlib4}.

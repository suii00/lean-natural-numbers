...
% --- タイトル情報 ---
\title{\textbf{構造塔理論による確率論の再構築} \\ \large --- Bourbaki形式主義からマルチンゲールまで ---}
\author{
...
数学的対象を個別に扱うのではなく、それらが属する「順序集合 (Poset)」や「階層構造」として捉えることが推奨される。
例えば、自然数の初期部分集合 $\{k \in \N \mid k \le n\}$ の族は、単なる集合の集まりではなく、包含関係によって順序付けられた構造をなす。
...
単なる単調な集合族に加え、「各要素が属する最小の層」が一意に定まることを要請する。
...
抽象的な構造塔理論を、測度論・確率論の基礎である $\sigma$-代数に応用する。
...
停止時間 $\Tau$ で止めたマルチンゲール $M^\Tau$ もまたマルチンゲールになる（有界な場合など）。これは、構造塔の射（停止操作）がマルチンゲールという「構造」を保存することを意味している。

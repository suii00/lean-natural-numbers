...
本プロジェクトの初期構想では、フィルトレーションの定義に追加の公理を含めることが検討されていた。これは「\textbf{幻の初期版フィルトレーション}」として記録に残る。

初期案では、以下の性質が考慮されていた:
\begin{itemize}
  \item 各層$\sigma_n$が有限生成であること
  \item 明示的な可算加法性の要求
  \item 極限層$\sigma_\infty := \bigcup_{n \in \mathbb{N}} \sigma_n$の存在
\end{itemize}

しかし、Bourbaki的な最小主義の精神に則り、最終版では単調性と被覆性のみを要求する形に落ち着いた。測度論的な詳細は、条件付き期待値構造の公理化により抽象化されている。

この変遷は、形式化における「どこまで公理化し、どこから構成的に示すか」というバランスの重要性を示している。

...
\begin{tikzpicture}[node distance=2.5cm, auto]
    ...
    \draw[->, thick] (LeExists) -- node[right, font=\footnotesize, align=left] {forget (Monotone $\phi$)} (D);
...
各ファイルには、定義が意図通りであることを確認するための簡単なテスト（Sanity check）が含まれています。これらはLeanでコンパイル可能な形（\texttt{1} は \texttt{\\mathbf{1}} として表示されるなど）に整えられています。

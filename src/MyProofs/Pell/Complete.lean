...
theorem pell_form_discriminant (D : ℕ) :
  ...
  unfold discriminant pell_form
  -- b² - 4ac where a=1, b=0, c=-D 
  -- = 0² - 4*1*(-D) = 0 + 4D = 4D
  norm_num
...
-- 完全平方数の例
example : IsSquare (4 : ℕ) := ⟨2, by norm_num⟩
...
theorem pell_2_exists : ∃ x y : ℤ, is_pell_solution 2 x y ∧ y ≠ 0 := by
  use 3, 2
  exact ⟨pell_2_solution, by norm_num⟩
...
/-
  ======================================================================
  Export for Import（主要定義は直接アクセス可能）
  ======================================================================
-/

-- namespace内の定義は PellComplete.is_pell_solution としてアクセス可能
-- namespace内の定理は PellComplete.pell_2_solution としてアクセス可能

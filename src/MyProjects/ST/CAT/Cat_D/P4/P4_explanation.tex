...
\begin{remark}
現状の定義（射を $(f,\phi)$ の\textbf{データ}として持つ）では、$\mathrm{forget}$ は一般に忠実（faithful）ではない：
同じ $f$ でも異なる $\phi$ を持つ射が同一の $f$ に写り得る。
一方で、$\mathrm{forget}$ は充満（full）ではない（$\mathbf{Cat}_{\mathcal{D}}$ の射の中には、$\mathbf{Cat}_{\mathrm{le}}$ に「持ち上げ」できないものが存在する）。
\end{remark}

...
Cat_D はその中でも存在量化のみを課す薄いバリアント（Cat_le, Cat_TowerD_Bij などの兄弟が想定される）。
...
- `Cat_eq.lean`: `minLayer` 等号版（`Cat_WithMin` の再輸出；`homeq` レーン）
- `Cat_TowerD_Bij.lean`: `TowerD` 上の bijective レーン（旧 `Cat_eq` の内容の退避先）
...
別ファイル（Cat_eq.lean = Cat_WithMin など）で関手として定式化される。
...
- 本ファイル → **Cat_eq.lean**: `minLayer` 等号版レーン（WithMin）
- 本ファイル → **Cat_TowerD_Bij.lean**: `TowerD` bijective レーン
...
2. **部分圏の構成**: HomLe（順序保存）、HomLeBij（bijective レーン）

...
example :
    (basisTower (K := ℚ) (V := Fin 2 → ℚ) (ι := Fin 2) (Pi.basisFun ℚ (Fin 2))).maxLayer =
      Module.finrank ℚ (Fin 2 → ℚ) := by
  simpa using
    maxLayer_basisTower_eq_finrank (K := ℚ) (V := Fin 2 → ℚ) (ι := Fin 2) (Pi.basisFun ℚ (Fin 2))

example :
    (basisTower (K := ℚ) (V := Fin 2 → ℚ) (ι := Fin 2)
          ((Pi.basisFun ℚ (Fin 2)).reindex (Equiv.swap (0 : Fin 2) (1 : Fin 2)))).maxLayer =
      Module.finrank ℚ (Fin 2 → ℚ) := by
  simpa using
    maxLayer_basisTower_eq_finrank (K := ℚ) (V := Fin 2 → ℚ) (ι := Fin 2)
      ((Pi.basisFun ℚ (Fin 2)).reindex (Equiv.swap (0 : Fin 2) (1 : Fin 2)))
...
/-! #### Rank = 1 concrete map (projection) -/

example :
    ((finBasisTower (K := ℚ) (V := LinearMap.range (LinearMap.fst ℚ ℚ ℚ))).maxLayer : ℕ) = 1 := by
  classical
  rw [maxLayer_finBasisTower_eq_finrank (K := ℚ) (V := LinearMap.range (LinearMap.fst ℚ ℚ ℚ))]
  have hrange : LinearMap.range (LinearMap.fst ℚ ℚ ℚ) = (⊤ : Submodule ℚ ℚ) := by
    simp [Submodule.range_fst (R := ℚ) (M := ℚ) (M₂ := ℚ)]
  have e :
      (LinearMap.range (LinearMap.fst ℚ ℚ ℚ)) ≃ₗ[ℚ] ℚ :=
    (LinearEquiv.ofEq (p := LinearMap.range (LinearMap.fst ℚ ℚ ℚ)) (q := (⊤ : Submodule ℚ ℚ))
        hrange).trans
      Submodule.topEquiv
  simpa using (LinearEquiv.finrank_eq e)

example :
    ((finBasisTower (K := ℚ) (V := LinearMap.ker (LinearMap.fst ℚ ℚ ℚ))).maxLayer : ℕ) = 1 := by
  classical
  have hker :
      LinearMap.ker (LinearMap.fst ℚ ℚ ℚ) = LinearMap.range (LinearMap.inr ℚ ℚ ℚ) := by
    simpa using (LinearMap.ker_fst (R := ℚ) (M := ℚ) (M₂ := ℚ))
  rw [maxLayer_finBasisTower_eq_finrank (K := ℚ) (V := LinearMap.ker (LinearMap.fst ℚ ℚ ℚ))]
  rw [hker]
  calc
    Module.finrank ℚ (LinearMap.range (LinearMap.inr ℚ ℚ ℚ)) = Module.finrank ℚ ℚ := by
      simpa using
        (LinearMap.finrank_range_of_inj (R := ℚ) (M := ℚ) (N := (ℚ × ℚ))
          (f := LinearMap.inr ℚ ℚ ℚ)
          (LinearMap.inr_injective (R := ℚ) (M := ℚ) (M₂ := ℚ)))
    _ = 1 := by simp

example :
    ((finBasisTower (K := ℚ) (V := LinearMap.range (LinearMap.fst ℚ ℚ ℚ))).maxLayer : ℕ) +
        ((finBasisTower (K := ℚ) (V := LinearMap.ker (LinearMap.fst ℚ ℚ ℚ))).maxLayer : ℕ) =
      ((finBasisTower (K := ℚ) (V := (ℚ × ℚ))).maxLayer : ℕ) := by
  simpa using
    rank_nullity_tower_finBasis (K := ℚ) (V := (ℚ × ℚ)) (V₂ := ℚ) (f := LinearMap.fst ℚ ℚ ℚ)

...
\begin{remark}
構造塔の「層」$\lambda(n)$は、従来の部分空間の階層とは異なる概念である。層$\lambda(n)$は「$n$個以下の基底要素で表現できるベクトル全体」であり、一般には部分空間ではない（複数の部分空間の合併となる）。この違いが構造塔理論の特徴であり、スパース表現や次元の段階的理解に有用である。
\end{remark}
...
\begin{remark}
構造塔は本質的に、ベクトル空間の「段階的な構成」を形式化したものである。各層$\lambda(n)$は「$n$個以下の基底ベクトルで表現できるベクトルの集合」であり、一般には部分空間ではない（例：$\mathbb{Q}^2$の$\lambda(1)$は2本の座標軸の合併で、加法で閉じていない）。
\end{remark}
...
構造塔の視点では、rank-nullity定理は「$V$の複雑さ（maxLayer）が、$f$によって失われる情報（$\ker f$）と保存される情報（$\mathrm{im}\, f$）の複雑さの和に等しい」という情報理論的解釈を持つ。

...
\begin{itemize}
  \item ベクトル空間の部分空間の階層：$\{0\} \subseteq V_1 \subseteq V_2 \subseteq \cdots \subseteq V_n = V$
  \item ベクトルの線型独立性と生成系
  \item 基底の最小性と次元の概念
...
\begin{remark}
構造塔は本質的に、ベクトル空間の「段階的な構成」を形式化したものである。各層$\lambda(n)$は「$n$個以下の基底ベクトルで生成される部分空間」と解釈できる。
\end{remark}
...
\begin{itemize}
  \item 点（0次元）$\to$ 直線（1次元）$\to$ 平面（2次元）$\to$ 空間（3次元）
\end{itemize}
...
Rank-nullity定理を構造塔の言語で表現することで、以下の幾何学的直観が得られる：

\begin{center}
\begin{tikzcd}
V \arrow[r, "f"] & W \\
\text{層} \, \lambda_V(n) \arrow[r] \arrow[d] & \text{層} \, \lambda_W(m) \arrow[d] \\
\ker f \oplus \mathrm{im}\, f \arrow[r, "\cong"] & V
\end{tikzcd}
\end{center}

この図式は、$V$の構造塔が$\ker f$と$\mathrm{im}\, f$の構造塔の「直和」として分解されることを示している。
...
この例は、$\mathbb{Q}^2$が$\ker f$（$y$軸）と$\mathrm{im}\, f$（$x$軸）に「分解」されることを構造塔の言語で表現している。

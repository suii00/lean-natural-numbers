...
theorem unramified_iff_e_one :
  ...
  · intro h
    have := K.ef_equals_degree
    have : K.ramification_index * K.degree = K.degree := by
      rw [← h]; exact this
    have : K.ramification_index = 1 := by
      have pos : 0 < K.degree := K.degree_pos
      have : K.ramification_index * K.degree = 1 * K.degree := by
        rw [this]; rfl
      exact Nat.eq_of_mul_eq_mul_right pos this
    exact this
...
noncomputable def unitGroupTower : StructureTowerMin UnitGroupData ℕ where
  layer := fun n => {data : UnitGroupData | data.unit_rank ≤ n}

  covering := by
    intro data
    exact ⟨data.unit_rank, le_refl _⟩

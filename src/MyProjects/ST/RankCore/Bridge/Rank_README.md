...
## Old `Cat_*` families: explainable correspondence (B4)

This project keeps the older tower categories intact. The Rank lane is the **canonical** one, and the
old categories become explainable through *views* and *adapters*.

| Old notion (tower side) | New notion (Rank side) | What it means | Bridge artifact |
|---|---|---|---|
| `TowerWithMin` / `StructureTowerWithMin` | `Ranked Index carrier` | take `rank := minLayer` | `rankedOfWithMin` |
| `homeq : T ⟶ T'` (minLayer equality lane) | `RankHomEq` | `minLayer (map x) = indexMap (minLayer x)` | `WithMin.toRankHomEq` |
| (via bridge) | `RankHomLe` | weaken equality to inequality | `WithMin.toRankHomLe` |
| (via bridge) | `RankHomD` | weaken to existence lane | `WithMin.toRankHomD` |
| old `Cat_le`-style morphism on WithMin objects | `RankHomLe` | layer preservation implies `minLayer(map x) ≤ indexMap(minLayer x)` | `OldHomLe.toRankHomLe` |

**Key point:** even if a morphism does **not** preserve `minLayer` by equality (old `Cat_le` intuition),
it still yields the **canonical `Le`-lane inequality** on ranks, as long as objects have `minLayer`.
This explains why the Rank lane takes `Le` as the main (stable) category of morphisms.

Lean file:
- `Rank/Bridge/FromOldCats.lean` (suggested path; see the generated skeleton `FromOldCats.lean`).

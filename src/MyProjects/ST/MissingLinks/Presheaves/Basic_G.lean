...
variable {I : Type u} [SemilatticeInf I] [OrderBot I]
...
structure TowerPresheaf (I : Type u) [Preorder I] where
  ...
  map_comp : ∀ {i j k : Iᵒᵖ} (f : i ⟶ j) (g : j ⟶ k), map (f ≫ g) = map g ∘ map f
...
def minFunctor (t : I) : I ⥤ I where
  ...
  map {n m} h := by
    -- n ≤ m implies n ⊓ t ≤ m ⊓ t
    exact homOfLE (@inf_le_inf_right I _ t n m (leOfHom h))
...
def stopped_presheaf (F : Functor Iᵒᵖ (Type v)) (τ : Ω → I) : Functor Iᵒᵖ (Type (max v w)) where
  ...
  map {n_op m_op} f := fun sections ω => by
  ...
  map_comp := by
    ...
    let y' := unop y
    let z' := unop z
    have hxy : y' ≤ x' := leOfHom f.unop
    have hyz : z' ≤ y' := leOfHom g.unop
    have hxz : z' ≤ x' := leOfHom (f ≫ g).unop
    -- F.map の合成を適用（反変なので順序に注意）
    have : F.map (homOfLE (inf_le_inf_right (τ ω) hxz)).op =
           F.map (homOfLE (inf_le_inf_right (τ ω) hxy)).op ≫
           F.map (homOfLE (inf_le_inf_right (τ ω) hyz)).op := by
      rw [←F.map_comp]
      congr 1
    rw [this]
    rfl
...
structure RightExtension {C D : Type*} [Category C] [Category D]
  (G : C ⥤ C) (F : C ⥤ D) where
  extension : C ⥤ D
  unit : F ⟶ G ⋙ extension -- 自然変換（修正: 正しい向き）

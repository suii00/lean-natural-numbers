...
theorem mul_assoc (a b c : D4Element) : (a * b) * c = a * (b * c) := by
  -- 全てのケースを明示的に検証
  simp only [Mul.mul, mul]
  cases a <;> cases b <;> cases c <;> rfl

/-- 逆元の性質 -/
theorem mul_left_inv (a : D4Element) : a⁻¹ * a = 1 := by
  cases a <;> simp only [Inv.inv, inv, Mul.mul, mul, One.one]
...
theorem r_pow_four : r^4 = 1 := by
  simp only [pow_succ, pow_zero, Mul.mul, mul]

theorem s_squared : s^2 = 1 := by
  simp only [pow_succ, pow_zero, Mul.mul, mul]

theorem braid_relation : s * r * s = r⁻¹ := by
  simp only [Mul.mul, mul, Inv.inv, inv]

/-- 中心の特徴付け -/
def is_central (a : D4Element) : Prop :=
  ∀ b : D4Element, a * b = b * a

theorem center_is_e_r2 : 
  ∀ a : D4Element, is_central a ↔ (a = e ∨ a = r2) := by
  intro a
  simp only [is_central]
  cases a <;> simp only [Mul.mul, mul] <;> {
    constructor
    · intro h
      -- 各元での可換性チェック
      specialize h r
      simp only [Mul.mul, mul] at h
      first | left; rfl | right; rfl | contradiction
    · intro h b
      cases h <;> cases b <;> simp only [Mul.mul, mul]
  }

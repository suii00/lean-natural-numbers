...
\begin{enumerate}
    ...
    \item $\minLayer$ の well-definedness には、線形性だけでなく各 $S_x$ の\textbf{最小元存在}が本質であり、整列集合では自動的に満たされる
...
\begin{enumerate}
    ...
    \item \textbf{最小性（Conditional Minimality）}：
    \[
        \forall x \in X,\ \bigl(\exists i \in I,\ x \in \Layer(i)\bigr) \Rightarrow
        \exists i_0 \in I,\ x \in \Layer(i_0) \land \forall j \in I,\ x \in \Layer(j) \Rightarrow i_0 \leq j
    \]
\end{enumerate}
\end{definition}

\begin{remark}[条件付き最小性の意義]
条件付き最小性は「$x$ がいずれかの層に属するならば、その中で最小の層が存在する」という主張である。
この定義では、被覆性を仮定とせずに最小性を述べるため、3公理の独立性が保証される。
\end{remark}
...
\begin{theorem}[$\minLayer$ の well-definedness]\label{thm:linear}
添字集合 $(I, \leq)$ が線形順序（全順序）であり、被覆性と単調性に加えて、
各 $x \in X$ について $S_x = \{i \in I \mid x \in \Layer(i)\}$ が最小元を持つとき、
$\minLayer$ は well-defined である。

特に、$(I, \leq)$ が整列集合（well-ordered set）であれば、条件は自動的に満たされる。
\end{theorem}

\begin{proof}
$x \in X$ を任意に取る。$\minLayer(x) = \min S_x$ と定義したい。
仮定より $S_x$ は最小元を持つので、これを $\minLayer(x)$ とすればよい。
最小元（least element）は、その定義から一意である（2つあれば互いに $\leq$ となり、反対称性で一致）。

注意として、線形順序であっても稠密順序（例：$\mathbb{Q}$）などの場合、
$S_x$ が最小元を持たないことがあり得る（後述の \texttt{denseLinearNoLeast} の例を参照）。
したがって「線形順序」だけでは十分ではなく、「各点での最小元の存在」が必要である。
\end{proof}
...
\node[order] (linear) at (0, 0) {整列集合\\（$\N$ など）};
\node[order] (dense) at (4, 0) {稠密線形順序\\（$\mathbb{Q}$ など）};
\node[order] (partial) at (8, 0) {半順序\\（$\N \times \N$ など）};

\node[font=\small, green!50!black] at (0, -1.5) {$\minLayer$ OK};
\node[font=\small, red!60] at (4, -1.5) {最小元なしの可能性};
\node[font=\small, red!60] at (8, -1.5) {比較不能の可能性};

\draw[ok] (linear.south) -- (0, -1);
\draw[ng] (dense.south) -- (4, -1);
\draw[ng] (partial.south) -- (8, -1);

\node[font=\bfseries] at (2.5, 1.5) {添字順序と $\minLayer$ の関係};
\end{tikzpicture}
\caption{添字順序と $\minLayer$：整列集合なら保証、稠密線形や半順序では失敗し得る}
...
% 注釈
\node[font=\small, text width=6cm, align=center] at (0, -2.5) {
    被覆＋単調だけでは最小性は出ない\\
    （線形かつ $S_x$ の最小元存在が必要）
};
\end{tikzpicture}
...
\begin{enumerate}
    ...
    \item 単調性 $\not\Rightarrow$ 被覆性（$\forall i,\, \Layer(i) = \emptyset$ で反例）
    \item 被覆性 $+$ 単調性 $+$ 「各 $S_x$ が least を持つ」 $\Rightarrow$ 最小性
    \item 被覆性 $+$ 単調性 $+$ 非線形順序 $\not\Rightarrow$ 最小性（rankNotUniqueTower）
    \item 被覆性 $+$ 単調性 $+$ 稠密線形順序 $\not\Rightarrow$ 最小性（denseLinearNoLeast）
...
\hline
rankNotUniqueTower & $\checkmark$ & $\checkmark$ & $\times$ & 積順序での比較不能性 \\
minLayerWithoutMonotone & $\checkmark$ & $\times$ & $\checkmark$ & 単調 $\not\Rightarrow$ 最小 \\
denseLinearNoLeast & $\checkmark$ & $\checkmark$ & $\times$ & 線形だが最小元なし（$\mathbb{Q}$） \\
emptyLayerTower & $\times$ & $\checkmark$ & $\checkmark^*$ & 被覆 $\not\Leftarrow$ 最小 \\
\hline
\multicolumn{5}{l}{\scriptsize $^*$条件付き最小性は空であるが真（前件偽）。codepthTower は $\N^{\mathrm{op}}$ を添字と見なせば単調。}
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{公理の独立性}：被覆性・単調性・最小性は互いに独立である。
    
    \item \textbf{添字順序の役割}：$\minLayer$ の well-definedness には、添字集合の線形性だけでは不十分で、
          各 $S_x$ が最小元を持つこと（整列性に近い性質）が必要である。
          $\mathbb{Q}$ のような稠密順序では反例が構成できる（\texttt{denseLinearNoLeast}）。
    
    \item \textbf{単調性と最小性の独立}：単調性がなくても最小性が成立する場合がある。
\end{enumerate}

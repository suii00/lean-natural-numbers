...
% 例の配置
\node[example, green!20] at (-2, 0) {pointTower\\全成立};
\node[example, green!20] at (6, 0) {intAbsTower\\全成立};
\node[example, red!20] at (-2, -1.5) {singletonLayer\\単調失敗};
\node[example, orange!20] at (6, -1.5) {rankNotUnique\\min非一意};

...
\author{su\thanks{Lean ソースコードに Codex (OpenAI) および本 \TeX 文書に AI ツール Antigravity (Google DeepMind) を使用して作成しました。}}
...
$(X, Y^*) \sim \text{BVN}(0, 0, 1, 1, \rho)$ のとき、$X = x$ を与えたときの $Y^*$ の条件付き分布は：
\begin{align}
    \E[Y^* \mid X = x] &= \rho x \\
...
\end{tikzpicture}
\caption{条件付きカテゴリ確率：$Y^* \mid X = x$ の分布における閾値間の面積}
...
theorem categoryProbability_sum_eq_one {k : Nat} (hk : 0 < k)
    ...
    sum j : Fin k, categoryProbability model x j = 1
\end{lstlisting}
